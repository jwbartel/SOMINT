...
public class NetworkFinder {
   ...
   public static Collection<Set<Integer>> doNetworkFind(Collection<Set<Integer>> cliques){
      ...
      while(mergeHappened){
    	 ...
    	 combineNetworkCliques(cliques, passNumber);
   ...
   private static void combineNetworkCliques(Collection<Set<Integer>> cliques0, int passNumber){
	  compareCount = 0;
			...
			for (Set<Integer> c2: cliques){
			   if(!toRemoveClique.get(innerIter) ){
				  if (!c1.equals(c2)){
					 ...
					 if(c1.size() >= c2.size()) 
						wasMerged = mergeNetworkCliques(c1, c2, iterationCount, innerIter);
					 else{
						firstOfCliquePairMerged = mergeNetworkCliques(c2, c1, innerIter, iterationCount);
						wasMerged = firstOfCliquePairMerged;
					 }
					 if(passNumber == -1){
						 if(wasMerged){
							 System.out.println(""+mergeCount+":merged"); 
						 }else{
							 System.out.println(""+mergeCount+":not merged");
						 }
					 }
					 mergeCount++;
