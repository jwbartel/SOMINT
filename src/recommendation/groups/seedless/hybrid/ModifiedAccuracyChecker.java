...
public class ModifiedAccuracyChecker {
	...
	public ModifiedAccuracyChecker(String id, String accuracyCompareOutFile) {
		participantId = id;
	...
	private static void findRLtoBestCostLists(ArrayList<FriendList> recLists,
			double[][] costMatrix) {
		for (int rl = 0; rl < recLists.size(); rl++) {
			...
			for (int il = 0; il < idealLists.size(); il++) {
				...
				if (costMatrix[rl][il] < 1) {
					bestLists = bestLists.concat(idealLists.get(il).name + " ("
							+ formatCost + ")" + ", ");
				}
			}
	...
	private static void printRLtoBestCostLists(ArrayList<FriendList> recLists,
			double[][] costMatrix) {
		System.out
				.println("~~~~~~~~~~~~Recommended Lists to Best Lists~~~~~~~~~~~~~~~~");
		...
		for (FriendList recList : recLists) {
			System.out.println(recList.name + ": "
					+ rlToBestList.get(recList.name));
		}
	}
	...
	private static void printRLtoBestListsKAcc(ArrayList<FriendList> recLists) {
		...
		for (FriendList recList : recLists) {
			System.out.println(recList.name + ": "
					+ rlToBestListKAcc.get(recList.name));
		}
	}
	...
	static int findJacobsCostToCreateIdeal(FriendList rl, FriendList il) {
		
		Set<String> intersect = new TreeSet<String>(rl.members);
		intersect.retainAll(il.members);
	...
	static double findKAccuracy(FriendList rl, FriendList il) {
		...
		for (String name : rl.members) {
			if (il.members.contains(name))
				numSame++;
		}
		if (rl.getSize() >= il.getSize()) {
			retVal = numSame / rl.getSize();
		} else
			retVal = numSame / il.getSize();
		/**
		 * if (RL(rl).size > IL(il).size) #RL in IL / RL(rl).size else #RL in IL
		 * / IL(il).size
		 */
		return retVal;
	}

	static void compareLists(ArrayList<FriendList> recLists,
			double[][] sMatrix, double[][] costMatrix, double[][] kAccMatrix) {
		...
		for (int rl = 0; rl < recLists.size(); rl++) {
			...
			for (int il = 0; il < idealLists.size(); il++) {
				...
				// AccuracyRecord record = new AccuracyRecord(costForIdeal,
				// costInsert, costKAcc, costDelete);
	 ...
	 * System.out.print("      "); // for (int il = 0; il < idealLists.size();
	 * il++){ // System.out.print(idealLists.get(il).name+" - "); // } //
	 * System.out.println(); // for(int rl=0; rl<recommendedLists.size(); rl++){
	 * // System.out.print(recommendedLists.get(rl).name+"  "); // for(int il=0;
	 * il<idealLists.size(); il++){ //
	 * System.out.print(accuracyMatrix1[rl][il].toFormattedString()+"  "); // }
	 * // System.out.println(); // } // for (int idealListIndex = 0;
	 * idealListIndex < idealLists.size(); idealListIndex++) { //
	 * ObjectEditor.setPropertyAttribute(accuracyMatrix1.getClass(), "*." +
	 * idealListIndex, AttributeNames.LABEL,
	 * idealLists.get(idealListIndex).name); //
	 * ObjectEditor.setPropertyAttribute(costMatrix.getClass(), "*." +
	 * idealListIndex, AttributeNames.LABEL,
	 * idealLists.get(idealListIndex).name); //
	 * ObjectEditor.setPropertyAttribute(kAccMatrix.getClass(), "*." +
	 * idealListIndex, AttributeNames.LABEL,
	 * idealLists.get(idealListIndex).name); // } // for (int recListIndex = 0;
	 * recListIndex < recommendedLists.size(); recListIndex++) { //
	 * ObjectEditor.setPropertyAttribute(accuracyMatrix1.getClass(), "" +
	 * recListIndex, AttributeNames.LABEL,
	 * recommendedLists.get(recListIndex).name); //
	 * ObjectEditor.setPropertyAttribute(costMatrix.getClass(), "" +
	 * recListIndex, AttributeNames.LABEL,
	 * recommendedLists.get(recListIndex).name); //
	 * ObjectEditor.setPropertyAttribute(kAccMatrix.getClass(), "" +
	 * recListIndex, AttributeNames.LABEL,
	 * recommendedLists.get(recListIndex).name); // } // // //
	 * ObjectEditor.setAttribute(Float.class, AttributeNames.COMPONENT_WIDTH,
	 * 60); // // comment the following line to see the full record //
	 * ObjectEditor.setAttribute(AccuracyRecord.class,
	 * AttributeNames.DEFAULT_EXPANDED, false); //
	 * ObjectEditor.setAttribute(AccuracyRecord.class,
	 * AttributeNames.ELIDE_STRING_IS_TOSTRING, true); //
	 ...
	 */
	static void printCostMatrix(double[][] costMatrix,
			ArrayList<FriendList> recLists) {
		try {
	...
	static void printJacobCostMatrix(int[][] jacobCostMatrix, ArrayList<FriendList> recLists) {
		try {
			PrintWriter pw = new PrintWriter(new FileWriter(outFile, true));
			pw.println("Recommended v Ideal: Jacob Cost Matrix");
			pw.print("   ");
			for (int il = 0; il < idealLists.size(); il++) {
				String ilName = idealLists.get(il).name;
				if (ilName.length() >= 3)
			...
			for (int rl = 0; rl < recLists.size(); rl++) {
				pw.print(recLists.get(rl).name + "  ");
				for (int il = 0; il < idealLists.size(); il++) {
					int cost = jacobCostMatrix[rl][il] - 1;
					DecimalFormat myFormatter = new DecimalFormat("#####");
					String formatCost = myFormatter.format(cost);
					if (cost >= 0) {
						if(formatCost.length() < 2){
							formatCost = "  "+formatCost+"   ";
						}else if(formatCost.length() < 3){
							formatCost = "  "+formatCost+"  ";
						}else{
							formatCost = " "+formatCost+"  ";
						}
	...
	Set<Integer> missingIdeals = new TreeSet<Integer>();
	
	Set<Point> getMinCostPos(int[][] jacobCosts, Set<Point> kPoints){
		missingIdeals.clear();
		
		Set<Point> retVal = new HashSet<Point>();
		
		for(int x=0; x<jacobCosts.length; x++){
			
			ArrayList<Point> listPoints = new ArrayList<Point>();
			for(Point point: kPoints){
				if(x == point.x) listPoints.add(point);
			}
			
			Point lowestCostPoint = null;
			int lowestCost = -1;
			for(Point point: listPoints){
				int cost = jacobCosts[point.x][point.y];
				if(lowestCostPoint == null || lowestCost >  cost){
					lowestCostPoint = point;
					lowestCost = cost;
				}
			}
			
			if(lowestCostPoint != null) retVal.add(lowestCostPoint);
		}
		
		for(int y=0; y<jacobCosts[0].length; y++){
			
			boolean found = false;
			for(Point point: retVal){
				if(point.y == y){
					found = true;
					break;
				}
	...
	void printTotalVals(int[][] adds, int[][] removes, Set<Point> points){
		int addSum = 0;
		int removeSum = 0;
		
		for(Point point: points){
			int add = adds[point.x][point.y];
			if(add >=0 ) addSum += add;
			
			int remove = removes[point.x][point.y];
			if(remove >=0) removeSum += remove;
		}
		
		try {
			PrintWriter pw = new PrintWriter(new FileWriter(outFile, true));
			pw.println("total adds, removes, missed ideals = "+addSum+", "+removeSum+", "+missingIdeals.size());
			pw.println("missing ideals:"+missingIdeals);
			pw.println();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	static void printPosInJacobCostMatrix(int[][] jacobCostMatrix, ArrayList<FriendList> recLists, Set<Point> minPositions, String valueTypes) {
		try {
			PrintWriter pw = new PrintWriter(new FileWriter(outFile, true));
			pw.println("Recommended v Ideal: "+valueTypes+" in Jacob Cost Matrix");
			pw.print("   ");
			for (int il = 0; il < idealLists.size(); il++) {
				String ilName = idealLists.get(il).name;
				if (ilName.length() >= 3)
			...
			for (int rl = 0; rl < recLists.size(); rl++) {
				...
				for (int il = 0; il < idealLists.size(); il++) {
					...
					DecimalFormat myFormatter = new DecimalFormat("#####");
					String formatCost = myFormatter.format(cost);
					if (minPositions.contains(new Point(rl, il))) {
						if(formatCost.length() < 2){
							formatCost = "  "+formatCost+"   ";
						}else if(formatCost.length() < 3){
							formatCost = "  "+formatCost+"  ";
	...
	void printRequiredManualAdds(){
		int totalManualAdds = 0;
		for(FriendList ideal : idealLists){
			totalManualAdds += ideal.getSize();
		}
		PrintWriter pw;
		try {
			pw = new PrintWriter(new FileWriter(outFile, true));
			pw.println("!!!!!!!!!REQUIRES "+totalManualAdds+" MANUAL ADDS IN NON-PREDICTIVE CASE!!!!!!!!");
			pw.println();
			pw.println();
			pw.close();
		} catch (IOException e) {
	...
	static void printkAccMatrix(double[][] kAccMatrix,
			ArrayList<FriendList> recLists) {
		try {
			PrintWriter pw = new PrintWriter(new FileWriter(outFile, true));
	...
	static ArrayList<FriendList> joinLists(
			ArrayList<FriendList> recommendedLists) {
		System.out.println("************** Join Lists ******************");
		ArrayList<FriendList> joinedLists = new ArrayList<FriendList>(
				recommendedLists);
		ArrayList<String> joinLists;
	...
	static void printSavingsMatrixToFile(String outputFile,
			double[][] savingsMatrix, ArrayList<FriendList> recommendedLists) {
		try {
			...
			for (int rl = 0; rl < recommendedLists.size(); rl++) {
				...
				for (int il = 0; il < idealLists.size(); il++) {
					DecimalFormat myFormatter = new DecimalFormat("0.000");
					String formatCost = myFormatter
							.format(savingsMatrix[rl][il]);
					if (savingsMatrix[rl][il] > 0) {
						pw.print(formatCost + " | ");
						// bestLists =
						// bestLists.concat(idealLists.get(il).name+" ("+formatCost+")"+", ");
						// System.out.println(bestLists);
					} else
						pw.print("      | ");
				}
				// rlToBestListKAcc.put(recommendedLists.get(rl).name,
				// bestLists);
				pw.println();
			}
		} catch (IOException e) {
	...
	static void setUpIdeal() {
		String idealInput = "data/Kelli/FriendshipData/ideal/" + participantId
				+ "_ideal.txt";
		String inLine = null;
	...
	static ArrayList<FriendList> setUpRecommendedNetworks() {
		...
		String idealInput = "data/Kelli/CompareResults/" + participantId
				+ "_LargeGroups.txt";
		String inLine = null;
		FriendList inList = null;
		try {
			...
			System.out.println("!!!setUpLists RECOMMENDED ERROR: "
					+ e.getMessage());
		}
	...
	@SuppressWarnings("deprecation")
	static ArrayList<FriendList> setUpRecommendedSubcliques(
			ArrayList<FriendList> recommendedLists) {
		ArrayList<FriendList> retList = new ArrayList<FriendList>(
				recommendedLists);
		String idealInput = "data/Kelli/CompareResults/" + participantId
				+ "_Subcliques.txt";
		String inLine = null;
		...
		try {
			...
			System.out.println("!!!setUpLists RECOMMENDED SUBCLIQUES ERROR: "
					+ e.getMessage());
		}
		return retList;
	}
	...
	static ArrayList<FriendList> setUpIntersection() {
		...
		try {
			...
			System.out.println("!!!setUpLists INTERSECTION ERROR: "
					+ e.getMessage());
		}
	...
	static ArrayList<FriendList> setUpSetDiff() {
		...
		try {
			...
			System.out
					.println("!!!setUpLists SETDIFF ERROR: " + e.getMessage());
		}
		return setDiffLists;
	}
	...
	public void compareLists(ArrayList<FriendList> recLists,
			double[][] costMatrix, double[][] kAccMatrix) {
		System.out.println("comparisons done here");
		for (int rl = 0; rl < recLists.size(); rl++) {
			FriendList recList = recLists.get(rl);
			for (int il = 0; il < idealLists.size(); il++) {
	...
	public void modifiedCompareLists(ArrayList<FriendList> recLists,
			double[][] costMatrix, 
			int[][] addsMatrix,
			int[][] removesMatrix,
			int[][] jacobCostMatrix,
		...
		for (int rl = 0; rl < recLists.size(); rl++) {
			...
			for (int il = 0; il < idealLists.size(); il++) {
				...
				FriendList idealList = idealLists.get(il);
				double costForIdeal = findCostToCreateIdeal(recList, idealList);
				int adds = findInsert(recList, idealList);
				int removes = findDelete(recList, idealList);
				int trueCostForIdeal = findJacobsCostToCreateIdeal(recList,
						idealList);
				double costKAcc = findKAccuracy(recList, idealList);
				costMatrix[rl][il] = costForIdeal;
				addsMatrix[rl][il] = adds;
				removesMatrix[rl][il] = removes;
				jacobCostMatrix[rl][il] = trueCostForIdeal + 1;
				if (costMatrix[rl][il] == 0) {
					String R = recLists.get(rl).name;
					dontJoinList.add(R);
				}
	...
	public Set<Point> getMinKCostPositions(double[][] kAccMatrix,  int[][] costs){
		
		Set<Point> retVal = new HashSet<Point>();
		
		for (int rl = 0; rl < kAccMatrix.length; rl++) {
			
			boolean kaccPrints = false;
			int minCost = -1;
			ArrayList<Integer> minCostILs = new ArrayList<Integer>();
			
			for (int il = 0; il < kAccMatrix[0].length; il++) {
				
				double kacc = kAccMatrix[rl][il];
				
				if (kacc > .5) {
					//kaccPrints = true;
					retVal.add(new Point(rl, il));
				}

				/*int cost = costs[rl][il];
				if(cost <= 0) continue;
				if(minCostILs.size() == 0 ||  minCost > cost){
					minCostILs.clear();
					minCostILs.add(il);
					minCost = cost;
	...
	public Set<Point> getMinCostPositions(int[][] costs){
		
		Set<Point> retVal = new HashSet<Point>();
		
		for(int rl = 0; rl < costs.length; rl++){
			int minCost = -1;
			ArrayList<Integer> minCostILs = new ArrayList<Integer>();
			for(int il=0; il < costs[rl].length; il++){
				int cost = costs[rl][il];
				if(cost <= 0) continue;
				if(minCostILs.size() == 0 ||  minCost > cost){
					minCostILs.clear();
